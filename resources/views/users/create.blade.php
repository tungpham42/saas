...
@section('title', __('Welcome New Member - SaaS AI Chatbot'))
...
<div class="max-w-3xl mx-auto">
    <div class="mb-8 animate-gentle">
        ...
        <h1 class="text-3xl font-bold text-amber-800 dark:text-amber-200">{{ __('Welcome a New Member 🎉') }}</h1>
        <p class="text-amber-600 dark:text-amber-400 mt-2">{{ __('Add someone special to our growing family') }}</p>
    </div>
    ...
    <div class="card-warm overflow-hidden animate-gentle" style="animation-delay: 0.1s">
        <div class="gradient-warm px-6 py-4">
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-amber-900 font-bold text-lg">{{ __('Member Details') }}</h2>
            </div>
            <p class="text-amber-800/70 text-sm mt-1">{{ __('Let\'s get to know them') }}</p>
        </div>
        ...
        <form action="{{ route('users.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" required value="{{ old('name') }}"
                           ...
                           placeholder="{{ __('e.g., Sarah Johnson') }}">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" required value="{{ old('email') }}"
                           class="input-warm w-full"
                           placeholder="{{ __('sarah@example.com') }}">
                    @error('email')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-semibold text-amber-700 dark:text-amber-300 mb-2">
                        <i class="fas fa-lock mr-2 text-amber-500"></i>{{ __('Password *') }}
                    </label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-amber-700 dark:text-amber-300 mb-2">
                        <i class="fas fa-lock mr-2 text-amber-500"></i>{{ __('Confirm Password *') }}
                    </label>
            ...
            <div class="border-t border-amber-100 dark:border-gray-700 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="mt-1 text-xs text-amber-500">{{ __('Base plan includes 1 bot. Extra bots are $2/month each.') }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="paypal_sub_status"
                                class="input-warm w-full">
                            <option value="None" {{ old('paypal_sub_status') == 'None' ? 'selected' : '' }}>{{ __('None') }}</option>
                            <option value="Active" {{ old('paypal_sub_status') == 'Active' ? 'selected' : '' }}>{{ __('Active 💝') }}</option>
                            <option value="Suspended" {{ old('paypal_sub_status') == 'Suspended' ? 'selected' : '' }}>{{ __('Suspended') }}</option>
                            <option value="Cancelled" {{ old('paypal_sub_status') == 'Cancelled' ? 'selected' : '' }}>{{ __('Cancelled') }}</option>
                        </select>
                    </div>
                </div>
            </div>
    ...
    <div class="mt-6 bg-amber-50 dark:bg-amber-900/20 rounded-2xl p-5 animate-gentle" style="animation-delay: 0.2s">
        <div class="flex items-start gap-3">
            ...
            <div>
                <h4 class="font-semibold text-amber-800 dark:text-amber-200">{{ __('💝 Spread Kindness') }}</h4>
                <p class="text-sm text-amber-600 dark:text-amber-400 mt-1">
                    {{ __('Every new member brings joy to our community. Welcome them warmly!') }}
                </p>
            </div>
        </div>
    </div>
